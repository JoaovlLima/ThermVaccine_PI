...
public class LoteService {
    ...
    public void exibirDados(Lote lote) {
        ...
        // talvez pegar apenas o nome do usuario e vacina, em vez de printar eles
        // inteiramente?
    ...
    public List<Lote> listarPorVacina(String idVacina) {
        return loteRepository.listar().stream()
                .filter(l -> l.getVacina().getId().equals(idVacina) && l.getQuantidade() > 0)
                .toList();
    }
    ...
    public int qtdVacPorLote(String idLote, List<Lote_coman> lote_comans) {
        int qtd = 0;
        for (Lote_coman lote_coman : lote_comans) {
            if (lote_coman.getLote().getId() == idLote)
                qtd += lote_coman.getQtd();
        }

        return qtd;
    }

    public void descontarLote(List<Lote_coman> lote_comans) {

        for (Lote_coman lote_coman : lote_comans) {
            Lote lote = lote_coman.getLote();
            lote.setQuantidade(lote.getQuantidade()- lote_coman.getQtd());
            loteRepository.editar(lote);
